...
//     return signal;
// }
const getSignal = function (cdl, levelOpt) {
    const priceOnLine = levelOpt.price;
    const spread = levelOpt.spread;
    let signal;
...
function LVL({ symbol, candles, levelOpt }) {
    ...
    //     signal = confirmSignal;
    // }
    if (cache[symbol].prelSignal == 'nextToBottom' &&
        curCdl.close < curCdl.open &&
        curCdl.close < levelOpt.price - levelOpt.spread) {
        signal = 'bounceDown';
    }
    else if (cache[symbol].prelSignal == 'nextToTop' &&
        curCdl.close > curCdl.open &&
        curCdl.close > levelOpt.price - levelOpt.spread) {
        signal = 'bounceUp';
    }
    else if (cache[symbol].prelSignal == 'crossBelow' &&
        curCdl.close < curCdl.open &&
        curCdl.close < levelOpt.price - levelOpt.spread) {
        signal = 'crossBelow';
    }
    else if (cache[symbol].prelSignal == 'crossAbove' &&
        curCdl.close > curCdl.open &&
        curCdl.close > levelOpt.price - levelOpt.spread) {
        signal = 'crossAbove';
    }
