...
function TDL({ candles, topLineOpt, bottomLineOpt }) {
    ...
    let signal = null;
    if (prevCdl.open < prevTopLinePrice &&
        prevCdl.close > prevTopLinePrice &&
        lastCdl.close > lastTopLinePrice) {
        signal = 'crossAboveTop';
    }
    else if (prevCdl.open > prevTopLinePrice &&
        prevCdl.close < prevTopLinePrice &&
        lastCdl.close < lastTopLinePrice) {
        signal = 'crossBelowTop';
    }
    else if (prevCdl.open > prevBottomLinePrice &&
    ...
    else if (prevCdl.high >= prevTopLinePrice &&
        // prevCdl.close < prevTopLinePrice &&
        prevCdl.open < prevTopLinePrice &&
        lastCdl.close < lastTopLinePrice) {
        signal = 'underTop';
    }
    else if (prevCdl.low <= prevBottomLinePrice &&
        // prevCdl.close > prevBottomLinePrice &&
        prevCdl.open > prevBottomLinePrice &&
        lastCdl.close > lastBottomLinePrice) {
        signal = 'overBottom';
    }
    else if (prevCdl.low <= prevTopLinePrice &&
        // prevCdl.close > prevTopLinePrice &&
        prevCdl.open > prevTopLinePrice &&
        lastCdl.close > lastTopLinePrice) {
        signal = 'overTop';
    }
    else if (prevCdl.high >= prevBottomLinePrice &&
        // prevCdl.close < prevBottomLinePrice &&
        prevCdl.open < prevBottomLinePrice &&
        lastCdl.close < lastBottomLinePrice) {
        signal = 'underBottom';
